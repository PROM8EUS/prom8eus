...
const AnimatedProgressBar: React.FC<{
  ...
  animated?: boolean;
  showPercentage?: boolean;
}> = ({ value, max, color, label, animated = true, showPercentage = true }) => {
  const { count } = useAnimatedCounter(animated ? value : value, 1500);
    ...
    <div className="space-y-2">
      <div className="flex justify-between items-center text-sm">
        ...
        <div className="flex items-center gap-2">
          <span className="font-semibold text-foreground">{count.toFixed(1)}h</span>
          {showPercentage && (
...
const ROIBlock: React.FC<{
  ...
  animated?: boolean;
  showTrends?: boolean;
}> = ({ savedHours, savedCost, period, lang, animated = true, showTrends = true }) => {
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-lg font-semibold text-foreground">
                {lang === 'de' ? 'Aufwand & ROI' : 'Effort & ROI'}
              </h3>
              <p className="text-xs text-muted-foreground">
                {lang === 'de' ? 'Automatisierungsanalyse' : 'Automation Analysis'}
              </p>
            </div>
          </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-xs text-muted-foreground">
                    ≈ {Math.round(animatedSavedCost).toLocaleString('de-DE')} €/{periodLabel} {lang === 'de' ? 'bei' : 'at'}
                  </div>
                </div>
