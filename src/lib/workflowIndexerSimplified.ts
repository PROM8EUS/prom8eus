...
export class SimplifiedWorkflowIndexer {
  ...
  /**
   * Get workflow by ID (ported from unified)
   */
  async getWorkflowById(id: string): Promise<UnifiedWorkflow | null> {
    try {
      const { data, error } = await supabase
        .from('unified_workflows')
        .select('*')
      ...
      if (error) {
        console.error('[WorkflowIndexer] Error fetching workflow by ID:', error);
        return null;
      }

      return data;
    } catch (error) {
      console.error('[WorkflowIndexer] Error in getWorkflowById:', error);
      return null;
    }
  }

  /**
   * Create new AI-generated workflow (ported from unified)
   */
  async createAIGeneratedWorkflow(context: any): Promise<UnifiedWorkflow | null> {
    try {
      const featureToggleManager = getFeatureToggleManager();
      const useWrite = featureToggleManager.isEnabled('unified_workflow_write');
      const useAIGeneration = featureToggleManager.isEnabled('unified_workflow_ai_generation');

      if (!useWrite || !useAIGeneration) {
        console.warn('[WorkflowIndexer] AI workflow generation not enabled via feature flags');
        return null;
      }

      // Generate basic workflow structure
      const workflow: UnifiedWorkflow = {
        id: `ai-indexer-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
        title: context.title || 'AI Generated Workflow',
        description: context.description || 'Generated by AI workflow indexer',
        source: 'ai-generated',
        category: context.category || 'automation',
        ...
        tags: context.tags || [],
        metadata: {
          generated_by: 'workflow-indexer-ai',
      ...
      console.error('[WorkflowIndexer] Failed to create AI-generated workflow:', error);
      return null;
    }
  }
